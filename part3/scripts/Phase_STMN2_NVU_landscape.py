...
Purpose: Map PT_STMN2 (TDP-43 pathology pseudotime) onto Phase 13 NVU framework
...
- Analysis: Mean PT_STMN2 per node × PT_dpt bin
"""
...
print("\nLoading PT_STMN2 results...")
tdp_file = IDS_DIR / "results" / "phaseTDP_STMN2" / "cell_level_features_with_PT_TDP_proper.csv"
df = pd.read_csv(tdp_file)

print(f"  Loaded {len(df):,} cells")
print(f"  Groups: {df['group'].value_counts().to_dict()}")

# Filter to ALS only with valid PT_STMN2 and PT_dpt
df_als = df[df['group'] == 'ALS'].copy()
df_als = df_als[df_als['PT_TDP'].notna() & df_als['PT_dpt'].notna()]

print(f"  ALS cells with PT_STMN2 & PT_dpt: {len(df_als):,}")
...
# =====================================================
# Compute PT_STMN2 profiles per NVU node
# =====================================================

print("\nComputing PT_STMN2(PT_dpt) profiles per NVU node...")
...
for node in ['Vascular', 'Glia', 'L2/L3', 'L3-L6', 'VAT1L']:
    ...
    for bin_idx in range(n_bins):
        ...
        profiles.append({
            ...
            'mean_PT_STMN2': mean_pt_tdp,
            'std_PT_STMN2': std_pt_tdp,
            'n_cells': n_cells
        })
...
# Save profiles
profile_file = RESULTS_DIR / "PT_STMN2_NVU_profiles.csv"
df_profiles.to_csv(profile_file, index=False)
print(f"  Saved: {profile_file.name}")

# =====================================================
# Visualization 1: PT_STMN2 trajectories per NVU node
# =====================================================

print("\nGenerating PT_STMN2 trajectory plot...")
...
for node in ['Vascular', 'Glia', 'L2/L3', 'L3-L6', 'VAT1L']:
    ...
    ax.plot(df_node['PT_dpt_center'], df_node['mean_PT_STMN2'],
    ...
    ax.fill_between(df_node['PT_dpt_center'],
                    df_node['mean_PT_STMN2'] - df_node['std_PT_STMN2'],
                    df_node['mean_PT_STMN2'] + df_node['std_PT_STMN2'],
                    alpha=0.2, color=colors[node])

ax.set_xlabel('PT_dpt (Disease Pseudotime)', fontsize=14, fontweight='bold')
ax.set_ylabel('PT_STMN2 (STMN2 Pathway Level)', fontsize=14, fontweight='bold')
ax.set_title('STMN2 Pathway Distribution Across NVU Nodes\n(Phase 13 Framework)',
             fontsize=16, fontweight='bold')
ax.legend(fontsize=11, loc='best', framealpha=0.9)
ax.grid(alpha=0.3, linestyle='--')

plt.tight_layout()
plt.savefig(RESULTS_DIR / 'Fig_PT_STMN2_NVU_trajectories.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"  Saved: Fig_PT_STMN2_NVU_trajectories.png")

# =====================================================
# Visualization 2: Heatmap (NVU nodes × PT_dpt bins)
# =====================================================

print("\nGenerating PT_STMN2 heatmap...")

# Pivot for heatmap
pivot_data = df_profiles.pivot(index='NVU_node', columns='PT_bin', values='mean_PT_STMN2')
...
sns.heatmap(pivot_data, cmap='RdYlBu_r', center=0.15,
            cbar_kws={'label': 'Mean PT_STMN2 (STMN2 Pathway)'},
...
ax.set_title('STMN2 Pathway Landscape: NVU Nodes × Disease Pseudotime\n(Phase 13 Framework)',
             fontsize=15, fontweight='bold')

plt.tight_layout()
plt.savefig(RESULTS_DIR / 'Fig_PT_STMN2_NVU_heatmap.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"  Saved: Fig_PT_STMN2_NVU_heatmap.png")
...
for node in ['Vascular', 'Glia', 'L2/L3', 'L3-L6', 'VAT1L']:
    ...
    summary_stats.append({
        ...
        'n_cells': len(df_node),
        'mean_PT_STMN2': mean_pt_tdp,
        'median_PT_STMN2': median_pt_tdp,
        'std_PT_STMN2': std_pt_tdp,
        'min_PT_STMN2': min_pt_tdp,
        'max_PT_STMN2': max_pt_tdp,
        'corr_PT_dpt_PT_STMN2': r_corr,
        'corr_pval': p_corr,
        'early_PT_STMN2': early_pt_tdp,
        'late_PT_STMN2': late_pt_tdp
    })

df_summary = pd.DataFrame(summary_stats)

# Save summary
summary_file = RESULTS_DIR / "PT_STMN2_NVU_summary_stats.csv"
df_summary.to_csv(summary_file, index=False)

print("\n  PT_STMN2 Statistics per NVU Node:")
...
# Rank by mean PT_STMN2
df_summary_sorted = df_summary.sort_values('mean_PT_STMN2', ascending=False)

print("\n1. STMN2 Pathway Severity (by mean PT_STMN2):")
for idx, row in df_summary_sorted.iterrows():
    print(f"   {row['NVU_node']}: {row['mean_PT_STMN2']:.3f} ± {row['std_PT_STMN2']:.3f}")

print("\n2. STMN2 Pathway vs Disease Progression Correlation:")
for idx, row in df_summary_sorted.iterrows():
    sig = "***" if row['corr_pval'] < 0.001 else "**" if row['corr_pval'] < 0.01 else "*" if row['corr_pval'] < 0.05 else "ns"
    print(f"   {row['NVU_node']}: r = {row['corr_PT_dpt_PT_STMN2']:.3f} ({sig})")

print("\n3. Early vs Late Disease Stage:")
for idx, row in df_summary_sorted.iterrows():
    if not np.isnan(row['early_PT_STMN2']) and not np.isnan(row['late_PT_STMN2']):
        change = row['late_PT_STMN2'] - row['early_PT_STMN2']
        direction = "↑" if change > 0 else "↓"
        print(f"   {row['NVU_node']}: {row['early_PT_STMN2']:.3f} → {row['late_PT_STMN2']:.3f} ({direction}{abs(change):.3f})")
